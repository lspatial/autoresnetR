...
#'
#' @param  corse: Number of CPU cores used for computing (default: 1)
#'
#' @param  cores: double, dropout rate for the target hidden layer (default 0) 
#'
...
ResautoDownscale=function(r2,fpredict0,c.grid,ss= 0.2,nepoch=30, cores= 1, thresh = 0.01,ntime=5){ 
  ...
  c.gridCELL=raster::rasterFromXYZ(cbind(xyFromCell(c.grid, cell=seq(1,ncell(c.grid)),spatial=FALSE),
                                 seq(1:ncell(c.grid))))
  ...
  c.datXY= raster::xyFromCell(c.grid, cell=seq(1,ncell(c.grid)), spatial=FALSE)
  ...
  for(i in c(1: (ntime+1))){ # i=1 
    ...
    rsquare=sptemExp::rSquared(xx$C_grid,xx$C_grid-xx$F_grid)[1,1]
    rmse=sptemExp::rmse(xx$C_grid,xx$F_grid)
    ...
    diogRMSE[i,"lowerCi"]= sqrt(lowerCi)
    diogRMSE[i,"upperCi"]= sqrt(upperCi)
    ...
    train_index=sample(c(1:nrow(vv)),size=ceiling(nrow(vv)*ss))
    ...
    pred = autoresmodel %>% keras::predict(x_test,batch_size = 2560)
    rquared=sptemExp::rSquared(y_test,y_test-pred)[1,1]
    rmse=sptemExp::rmse(y_test,pred)
    predfull = autoresmodel %>% predict(vvfull[,c(3:tlen)],batch_size = 2560)
